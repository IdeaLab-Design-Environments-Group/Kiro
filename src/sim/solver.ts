...
export class FoldSolver {
  ...
  constructor(readonly model: BarHingeModel, dtScale = 1) {
    // dtScale < 1 adds stability margin: computeDt only bounds the axial mode,
    // so a free (unguided) mesh with crease+face springs needs extra headroom.
    this.dt = computeDt(model) * dtScale;
