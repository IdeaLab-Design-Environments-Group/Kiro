...
/**
 * Stable timestep dt = 0.9/(2π·ω_max). ω_max is the highest natural frequency of *any* mode,
 * not just axial. The paper assumes k_axial dominates, but here k_crease = k_fold·l0 grows with
 * edge length while k_axial = EA/l0 shrinks, so on a normalized (large-l0) mesh the **crease
 * bending mode is the stiffest** — and sizing dt from axial alone lets explicit Euler inject
 * energy every step (the jitter). The crease's effective linear stiffness is k_crease/h², where
 * h is the wing's moment arm to the crease line (small for thin molecule triangles); we evaluate
 * it on the flat rest mesh and floor h at 3% of the crease length so a near-degenerate triangle
 * can't drive dt to zero.
 */
export function computeDt(m: BarHingeModel): number {
  let maxFreq2 = 0; // ω², over all modes
  // axial bars
  for (let i = 0; i < m.beams.count; i++) {
    const mMin = Math.min(m.mass[m.beams.n0[i]], m.mass[m.beams.n1[i]]);
    const w2 = m.beams.k[i] / mMin;
    if (w2 > maxFreq2) maxFreq2 = w2;
  }
  // crease bending: effective linear stiffness k_crease / h² on the rest mesh
  const rest = m.rest;
  for (let i = 0; i < m.creases.count; i++) {
    const p3 = get(rest, m.creases.n3[i]);
    const p4 = get(rest, m.creases.n4[i]);
    const e = sub(p4, p3);
    const el = len(e) || 1e-9;
    const eh = { x: e.x / el, y: e.y / el, z: e.z / el };
    const hFloor = 0.03 * el;
    for (const wing of [m.creases.n1[i], m.creases.n2[i]]) {
      const r = sub(get(rest, wing), p3);
      const proj = dot(r, eh);
      const h = Math.max(Math.sqrt(Math.max(0, dot(r, r) - proj * proj)), hFloor);
      const mMin = Math.min(m.mass[wing], m.mass[m.creases.n3[i]], m.mass[m.creases.n4[i]]);
      const w2 = m.creases.k[i] / (h * h) / mMin;
      if (w2 > maxFreq2) maxFreq2 = w2;
    }
  }
  if (maxFreq2 <= 0) return 1e-3;
  return (1 / (2 * Math.PI * Math.sqrt(maxFreq2))) * 0.9;
}
