 ...
 *
 * Traces follow the same cloth-gap routing as the sim (they thread the spaces between the tiles, never
 * jumping a hinge); each routed segment becomes a low rectangular rib. Parts: a body box + two SMD pad
 * boxes sized from the fab.pretty footprint. All flat (z = 0 base, extruded up), sized from the model
 * bbox so they read at any pattern scale.
 */
import { type Circuit } from "./circuit.js";
import { type MeshRef, resolveCircuit, TRACE_W, type Vec3 } from "./circuit-geometry.js";
...
/** Trace rib height and pad / body extrude height, as fractions of the bbox diagonal (rib width = TRACE_W). */
const RIB_H = 0.012;
const PAD_H = 0.018;
const BODY_H = 0.03;
...
export function buildCircuitStl(circuit: Circuit, net: MeshRef, baseName = "kirigami"): CircuitStl | null {
  ...
  const d = geo.scale;
  const ribW = TRACE_W * d, ribH = RIB_H * d, padH = PAD_H * d, bodyH = BODY_H * d;

  const out: string[] = [`solid ${baseName}-circuit`];

  // Traces → a rib per routed segment (box along the segment, width ribW, z 0..ribH).
  ...
  // Parts → body + two copper pads (flat: x,y in-plane, extrude in z).
  for (const c of geo.components) {
    const x = flat2(c.x), y = flat2(c.y);
    boxFrame(out, flatXY(c.center), x, y, c.len / 2, c.wid / 2, 0, bodyH);
    for (const p of c.pads) boxFrame(out, flatXY(p), x, y, c.padLen / 2, c.padWid / 2, 0, padH);
  }
